...
[![100% AI-crafted](https://img.shields.io/badge/100%25-AI--crafted-9cf.svg)](#免责声明)

**把文件推送到 GitHub——无需 git，省 token、护隐私、跨平台、多语言。**

`gh-publisher` 是一个智能体技能：用 `gh` CLI + GitHub REST API（Contents / Git Database）把本地目录发布到 GitHub 仓库——不需要 git。它用一条可复用脚本自动完成空仓库初始化与批量提交，让智能体一条命令就推完，而不必逐文件重推 API 细节。它还发布 **GitHub 最常用的 10 种语言**（README 翻译版），而本地工作副本始终保留你选定的语言。
...
- **🌍 多语言发布**——本地工作副本保留你选定的语言（默认中文）；发布版附带 GitHub 最常用 10 种语言的 README 翻译（en、zh-CN、hi、es、fr、ar、bn、pt、ru、ja）。见 `references/i18n.md`。

## 工作原理

1. `pwsh -ExecutionPolicy Bypass -File scripts/push.ps1 -Source <目录> -Repo owner/repo -Message "…" [-GhPath <gh.exe 路径>] [-Languages en,zh-CN,hi,es,fr,ar,bn,pt,ru,ja]`
...
7. 可选 `-Languages` 检查：推送前校验每种语言文件存在（如 `README.hi.md`），缺失即告警。

## 多语言发布（本地语言 + 10 种发布语言）

- **本地工作副本**：保留你配置的语言（zh 或 en）——配置存于已安装技能目录的 `config.local.json`（默认 `zh`）。说「更改本地保留默认语言为英文」即可切换，本地 SKILL.md 会同步更新。
- **发布版**：`SKILL.md` 保持英文（GitHub 通用主版本），`README.<lang>.md` 发布 10 种最常用语言。语言清单、翻译规范与触发契约注意事项见 `references/i18n.md`（所有语言版本 `name` 保持一致）。
